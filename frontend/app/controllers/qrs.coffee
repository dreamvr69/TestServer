...
QrsController = Ember.Controller.extend ToastMixin,
  ...
  actions:
    ...
    show: (qr) ->
      @transitionToRoute 'qr', qr.id

    edit: (qr) ->
      @transitionToRoute 'edit-qr', qr.id

    destroy: (qr) ->
      if confirm('Удалить полезное действие?')
        qr.destroyRecord()
        @showToast('Полезное действие успешно удалено!')

`export default QrsController`
