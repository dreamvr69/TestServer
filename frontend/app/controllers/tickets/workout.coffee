...
TicketsWorkoutController = Ember.Controller.extend TicketBaseControllerMixin, ToastMixin,
  ...
  actions:
    close: (ticket)->
      $.ajax
        ...
        url: "#{config.protocol}://#{config.host}/ember/ticket/workout_requests/#{ticket.id}/close"
        success: (data) =>
    ...
    decline: (ticket)->
      $.ajax
        ...
        beforeSend: (request) =>
          ...
          token = @get('session.data.authenticated.token')
          request.setRequestHeader 'Authorization', 'Token token="'+token+'", id="'+id+'"'
        url: "#{config.protocol}://#{config.host}/ember/ticket/workout_requests/#{ticket.id}/decline"
        success: (data) =>
...
`export default TicketsWorkoutController`
