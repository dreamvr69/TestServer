...
TicketsMessageController = Ember.Controller.extend TicketBaseControllerMixin, ToastMixin,
  ...
  actions:
    close: (ticket)->
      $.ajax
        ...
        url: "#{config.protocol}://#{config.host}/ember/ticket/messages/#{ticket.id}/close"
        success: (data) =>
...
`export default TicketsMessageController`
